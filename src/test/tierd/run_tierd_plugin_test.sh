...
APPNAME=tierd
UPROF=$TIERDDIR/AMDuProf_Linux_x64_4.0.341/bin/AMDuProfPcm
PCM=$TIERDDIR/pcm/build/bin/pcm-memory
...
function check_binary() {
	if [ "$CPU_VENDOR" = "AuthenticAMD" ]; then
		if [ ! -f $UPROF ]; then
			log_error "AMDuProfPCM binary does not exist. Run 'build_lib.sh tierd' in /path/to/SMDK/lib/."
			exit $ENV_SET_FAIL
		elif [ ! -f $PCM ]; then
			log_error "Intel PCM binary does not exist. Run 'build_lib.sh tierd' in /path/to/SMDK/lib/."
			exit $ENV_SET_FAIL
		fi
	fi
}
