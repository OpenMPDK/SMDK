...
STRING_NORMAL_ZONE_SIZE_UNLIMITED="normal_size = unlimited"
STRING_EXMEM_ZONE_SIZE_UNLIMITED="exmem_size = unlimited"
...
function check_tierd_running() {
	tierd_run_count=$(ps uafx | grep -x "\<tierd\>" | grep -v grep | wc -l)
	if [ $tierd_run_count -ne 0 ]; then
		echo "Stop tierd daemon and try again"
		exit $ENV_SET_FAIL
	fi
}
...
function check_binaries() {
	if [ ! -f "$SMDKLIB_PATH" ]; then
		echo "libcxlmalloc.so does not exist. Run 'build_lib.sh libsmdk' in /path/to/SMDK/lib"
		exit $ENV_SET_FAIL
	fi
}
...
function tierd_mock_start() {
	...
	for number in "${nids[@]}"; do
		NODE_DIR_PATH=/run/tierd/node$number/
		sudo mkdir $NODE_DIR_PATH
		echo -n true | sudo tee $NODE_DIR_PATH/idle_state_bandwidth > /dev/null
	done
}

function tierd_mock_end() {
	sudo rm -rf /run/tierd/
}
...
function t1() {
	# Without /run/tierd, adaptive interleaving should be disabled
...
function t2() {
	# If adaptive interleaving is enabled, normal_size should be ignored
	tierd_mock_start
	CXLMALLOC_CONF=use_exmem:true,use_adaptive_interleaving:true,normal_size:1G
	OUTPUT_FILE=t2.log
	run_testcase $CXLMALLOC_CONF $OUTPUT_FILE
...
function t3() {
	# If adaptive interleaving is enabled, exmem_size should be ignored
	tierd_mock_start
	CXLMALLOC_CONF=use_exmem:true,use_adaptive_interleaving:true,exmem_size:1G
	OUTPUT_FILE=t3.log
	run_testcase $CXLMALLOC_CONF $OUTPUT_FILE
