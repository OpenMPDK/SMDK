...
# - Note that there's no CXL Cache's page in CXL memory, ENV_SET_FAIL is returned.
#   There's no problem to run flush itself in this case,
#   but this TC only targets flush out CXL Cache's pages in CXL memory.
...
#                - There's no CXL Cache's page in CXL memory 
...
if [ -e $DEBUGFS_EXIST ]; then
	...
	if [ $PUT_PAGES -eq 0 ]; then
		echo "There's no CXL Cache's page in CXL memory"
		exit $ENV_SET_FAIL
	fi
