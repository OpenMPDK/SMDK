...
void usage(void)
{
	fprintf(stderr,
#ifdef SUPPORT_SMDK_KERNEL
		"usage: numactl [--all | -a] [--zone= | -z <zone>]\n"
		"               [--interleave= | -i <nodes>] [--preferred= | -p <node>]\n"
#else
...
void nopolicy(void)
{
	if (set_policy >= 0)
#ifdef SUPPORT_SMDK_KERNEL
		usage_msg("specify policy after --shm/--file/--zone");
#else
...
int main(int ac, char **av)
{
	...
	while ((c = getopt_long(ac, av, shortopts, opts, NULL)) != -1) {
		...
		case 'z': /* --zone */
			nopolicy();
			if (strlen(optarg) != 1 || (optarg[0] != 'e' && optarg[0] != 'n')) {
				printf ("<%s> is invalid\n", optarg);
				usage();
			}
			if (optarg[0] == 'e')
