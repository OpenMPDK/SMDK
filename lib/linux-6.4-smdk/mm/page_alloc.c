...
#ifdef CONFIG_EXMEM
#define ZONELIST_ORDER_NORMAL	0
#define ZONELIST_ORDER_EXMEM	1

static char zonelist_order_name[2][NUMA_ZONELIST_ORDER_LEN] = {
	"Normal",
	"ExMem"
};
static int user_zonelist_order = ZONELIST_ORDER_NORMAL;
	...
	for (i = 0; i < MAX_NR_SUBZONES; i++) {
		if (!zone->free_area_subzone[i]) {
			zone->free_area_subzone[i] =
				kzalloc(sizeof(struct free_area) * (MAX_ORDER + 1), GFP_ATOMIC);
			if (!zone->free_area_subzone[i])
				goto alloc_failed;
		}
	}
			 ...
			 long free_pages)
{
	...
	if (zone_is_zone_exmem(z)) {
		...
		for_each_subzone(i, subzone_idx, z) {
			...
			if (__check_suitable_free_page(area, alloc_flags))
					return true;
			}
		}
	} else {
...
static inline bool __is_zone_allowed(struct zone *z, gfp_t gfp_mask,
		int alloc_flags)
{
	if (numa_zonelist_order == ZONELIST_ORDER_NORMAL &&
			(alloc_flags & ALLOC_NOFRAGMENT) && zone_is_zone_exmem(z))
		return false;
...
#ifdef CONFIG_EXMEM
		if (!__is_zone_allowed(zone, gfp_mask, alloc_flags))
			continue;
#endif
...
#ifdef CONFIG_EXMEM
		if (!__is_zone_allowed(zone, gfp, alloc_flags))
			continue;
#endif
...
static int build_zonerefs_node(pg_data_t *pgdat, struct zoneref *zonerefs)
{
	...
	do {
		zone_type--;
#ifdef CONFIG_EXMEM
		if (user_zonelist_order == ZONELIST_ORDER_NORMAL
				&& zone_type == ZONE_EXMEM)
			continue;
#endif
		...
		if (user_zonelist_order == ZONELIST_ORDER_NORMAL
				&& !zone_type) {
			zone = pgdat->node_zones + ZONE_EXMEM;
			if (populated_zone(zone)) {
				zoneref_set_zone(zone, &zonerefs[nr_zones++]);
				check_highest_zone(zone_type);
			}
...
{
#ifdef CONFIG_EXMEM
	if (!strncasecmp(s, zonelist_order_name[ZONELIST_ORDER_NORMAL],
...
#ifdef CONFIG_EXMEM
char numa_zonelist_order[] = "Normal";
#else
