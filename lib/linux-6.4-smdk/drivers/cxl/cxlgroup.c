...
extern void set_exmem_range(unsigned long start_pfn, unsigned long end_pfn);
...
static struct cxl_memblk *add_cxl_meminfo(u64 start, u64 end, int node_id, bool from_srat)
{
	...
	set_exmem_range(start >> PAGE_SHIFT, (end + 1) >> PAGE_SHIFT);
...
static int register_cxl_cfmws_ranges(struct cxl_decoder *cxld)
{
	...
	rc = update_or_add_cxl_meminfo(cxld->hpa_range.start,
			cxld->hpa_range.end, dev->numa_node);
	if (rc)
		return rc;
