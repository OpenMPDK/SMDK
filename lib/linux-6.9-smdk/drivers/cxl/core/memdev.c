...
#ifdef CONFIG_EXMEM
static ssize_t dvsec_ranges_show(struct device *dev,
				 ...
				 char *buf)
{
	...
	if (rc) {
		dev_err(dev, "Failed to get DVSEC range information (%d)\n", rc);
		return sysfs_emit(buf, "\n");
	}

	dev_dbg(dev, "dvsec ranges count: %d\n", info.ranges);

	for (i = 0; i < info.ranges; i++)
		if (info.dvsec_range[i].start > 0)
			len = sprintf(buf + strlen(buf), "%08llx-%08llx\n",
					info.dvsec_range[i].start, info.dvsec_range[i].end);

	return len > 0 ? len : sysfs_emit(buf, "\n");
}
static DEVICE_ATTR_RO(dvsec_ranges);
#endif
