...
static int cxl_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	for (i = 0; i < hmu_count; i++) {
		...
		rc = devm_cxl_hmu_add(cxlds->dev, &hmu_regs, cxlmd->id, i, CXL_HMU_MEMDEV);
		if (rc) {
	...
	/*
	 * FIXME: PNM POC may not be supporting event record.
	 */
	rc = cxl_event_config(host_bridge, mds, irq_avail);
	if (rc && !(cxl_is_imdb(pdev) || cxl_is_sls(pdev)))
		return rc;
	...
	if (cxl_is_sls(pdev)) {
		dev_dbg(&pdev->dev, "Detected SLS device");
		rc = cxl_add_sls(&pdev->dev, cxlmd->id);
	}
