...
static int mpol_new_nodemask(struct mempolicy *pol, const nodemask_t *nodes)
{
	...
	if (pol->mode == MPOL_WEIGHTED_INTERLEAVE2)
...
static long replace_mempolicy(struct mempolicy *new, nodemask_t *nodes)
{
	...
	if (new && (new->mode == MPOL_INTERLEAVE ||
		    ...
		    new->mode == MPOL_WEIGHTED_INTERLEAVE2)) {
...
static void get_policy_nodemask(struct mempolicy *pol, nodemask_t *nodes)
{
	...
	case MPOL_WEIGHTED_INTERLEAVE2:
		*nodes = pol->nodes;
		break;
	case MPOL_LOCAL:
...
static long do_get_mempolicy2(struct mempolicy_args *kargs)
{
	...
	if (kargs->get.addr.addr) {
		...
		if (vma->vm_ops && vma->vm_ops->get_policy)
			addr_pol = vma->vm_ops->get_policy(vma, addr, &ilx);
		else
...
static unsigned int weighted_interleave_nodes2(struct mempolicy *policy)
{
	unsigned int next;
...
static unsigned int interleave_nodes(struct mempolicy *policy)
{
	...
	if (policy->mode == MPOL_WEIGHTED_INTERLEAVE2)
		return weighted_interleave_nodes2(policy);
...
static inline unsigned interleave_nid2(struct mempolicy *pol,
		...
		struct mempolicy **mpol, nodemask_t **nodemask)
{
	...
	if (unlikely((*mpol)->mode == MPOL_WEIGHTED_INTERLEAVE2)) {
		nid = interleave_nid2(*mpol, vma, addr,
		...
		unsigned long addr, bool hugepage)
{
	...
	if (pol->mode == MPOL_WEIGHTED_INTERLEAVE2) {
		...
		nid = interleave_nid2(pol, vma, addr, PAGE_SHIFT + order);
		mpol_cond_put(pol);
		gfp |= __GFP_COMP;
...
static unsigned long alloc_pages_bulk_array_weighted_interleave2(gfp_t gfp,
	struct mempolicy *pol, unsigned long nr_pages,
	struct page **page_array)
{
	struct task_struct *me = current;
	...
	rounds = rem_pages / pol->wil.il_weight;
	delta = rem_pages % pol->wil.il_weight;
	for (i = 0; i < nnodes; i++) {
		...
		unsigned long nr_pages, struct page **page_array)
{
	...
	if (pol->mode == MPOL_WEIGHTED_INTERLEAVE2)
		return alloc_pages_bulk_array_weighted_interleave2(gfp, pol,
								  nr_pages,
								  page_array);
...
static const char * const policy_modes[] =
{
	...
	[MPOL_WEIGHTED_INTERLEAVE2] = "weighted interleave 2",
	[MPOL_LOCAL]      = "local",
