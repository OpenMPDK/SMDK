			...
			unsigned long *handle)
{
	...
	nodemask_t memonly_nodes = NODE_MASK_NONE;
	nodes_andnot(memonly_nodes, node_states[N_MEMORY], node_states[N_CPU]);
	page = __alloc_pages(gfp, 0, numa_node_id(), &memonly_nodes);
	if (!page)
		return -ENOMEM;
