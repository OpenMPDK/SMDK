...
struct exmem_range_info {
	int nr_ranges;
	struct exmem_range range[MAX_NUMNODES];
};

static struct exmem_range_info exmem_info;

void set_exmem_range(unsigned long start_pfn, unsigned long end_pfn)
{
	exmem_info.range[exmem_info.nr_ranges].start_pfn = start_pfn;
	exmem_info.range[exmem_info.nr_ranges].end_pfn = end_pfn;
	exmem_info.nr_ranges++;
}
EXPORT_SYMBOL_GPL(set_exmem_range);

static bool exmem_zone_intersects(unsigned long start_pfn, unsigned long nr_pages)
{
	int i = 0;

	for (i = 0 ; i < exmem_info.nr_ranges; i++) {
		if (start_pfn >= exmem_info.range[i].start_pfn &&
		     start_pfn + nr_pages <= exmem_info.range[i].end_pfn)
			return true;
	}
...
#ifdef CONFIG_EXMEM
	if (exmem_zone_intersects(start_pfn, nr_pages))
...
#ifdef CONFIG_EXMEM
	present_pages += pgdat->node_zones[ZONE_EXMEM].present_pages;
#endif
