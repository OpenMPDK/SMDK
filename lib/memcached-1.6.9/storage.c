...
int storage_read_config(void *conf, char **subopt) {
    ...
    switch (getsubopt(subopt, subopts_tokens, &subopts_value)) {
        ...
        case EXT_DIRECT_IO:
            if (subopts_value == NULL) {
                fprintf(stderr, "Missing ext_direct_io argument\n");
                return 1;
            }
            if (!safe_strtoul(subopts_value, &settings.ext_direct_io)) {
                fprintf(stderr, "could not parse argument to ext_direct_io\n");
                return 1;
            }
            break;
        case EXT_PATH:
