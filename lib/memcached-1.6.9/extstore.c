        ...
        while (cur_io) {
            ...
            switch (cur_io->mode) {
                ...
                case OBJ_IO_WRITE:
                    ...
                    // Note: when direct io is setted, bypass evict written data from page cache
                    if(ret > 0 && settings.ext_direct_io == 1){
                        fsync(p->fd);
                        posix_fadvise(p->fd,p->offset+cur_io->offset, cur_io->len, POSIX_FADV_DONTNEED);
                    }
                    break;
            }
            if (ret == 0) {
